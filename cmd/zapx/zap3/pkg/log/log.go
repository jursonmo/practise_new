...
func New(writer io.Writer, level Level, opts ...Option) *Logger {
	...
	//对于zapcore来说，日志级别方面，只需要实现LevelEnabler接口就行，
	//它的作用就是，传入一个level int， 返回true or false,
	//返回true表示要打印出来，false 表示不要打印
	//zapcore.Level类型实现了LevelEnabler接口，但是它比较固定
	//传入zap.NewAtomicLevel 就可以动态修改
	core := zapcore.NewCore(
		zapcore.NewJSONEncoder(cfg.EncoderConfig),
		zapcore.AddSync(writer),
		zapcore.Level(level), //zapcore.Level类型实现了LevelEnabler接口
	)
	logger := &Logger{
		l:     zap.New(core, opts...),
		level: level,
